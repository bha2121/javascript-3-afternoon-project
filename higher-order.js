...
//Code Here
let evenNumbers = mixedNumbers.filter(item => item %2 === 0)   // = mixedNumbers.filter(/* Provide Your Callback Here */)
...
//Code Here
let postTaxPrices = prices.map(item => item * 1.07)// = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation = populations.reduce((total, current) => total + current) //  = populations.reduce(/* Provide Your Callback Here */)
...
//Code Here
let myStrongest = monstersInYourPocket.filter(item => item.CP > 200) // = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
// Code here
let orderTotals = orders.map(item => item.price * (1 + item.tax ))
...
*/
 ...
 //Code Here
let bobsPurchases = purchases.filter(item => item.owner.includes('Bob'))

console.log(bobsPurchases)
console.log(bobsPurchases[0].price)

let bobsTotal = bobsPurchases.reduce((total, current) => total + current.price, 0)
